...
import { REACTION_ICONS, REACTION_LABELS } from "../../components/Icons";
...
function ReactionPickerItem({ emoji, onSelect, onClose }) {
  const IconComponent = REACTION_ICONS[emoji];
  return (
    <button
      onClick={() => { onSelect(emoji); onClose(); }}
      className="flex flex-col items-center gap-0.5 p-1.5 rounded-xl hover:bg-brand-50 hover:scale-125 transition-all duration-100"
      title={REACTION_LABELS[emoji] || emoji}
      aria-label={REACTION_LABELS[emoji] || emoji}
    >
      {IconComponent ? <IconComponent className="w-5 h-5 text-gray-600" /> : <span className="text-sm">{emoji}</span>}
    </button>
  );
}
...
function EmojiPicker({ onSelect, onClose, align = "left" }) {
  ...
  return (
    ...
    >
      {EMOJI_LIST.map((e) => (
        <ReactionPickerItem key={e} emoji={e} onSelect={onSelect} onClose={onClose} />
      ))}
    </div>
  );
}
        ...
        Object.entries(reactions).map(([emoji, users]) => {
          ...
          return users.length > 0 ? (
            <button
              ...
              aria-label={`${REACTION_LABELS[emoji] || emoji}: ${users.length}`}
            ...
            >
              {IconComponent ? <IconComponent className="w-3.5 h-3.5" /> : <span className="text-xs">{emoji}</span>}
      ...
      {canReact && (
        <div className="relative">
          <button
            ...
            className="inline-flex items-center justify-center w-6 h-6 rounded-full text-gray-700 hover:bg-gray-100 hover:text-brand-500 transition-colors"
            title="Réagir"
          ...
          <div className="flex-shrink-0 mt-2" style={{ animation: "slideUp 0.2s ease" }}>
            {error && (
              <div className="mb-2 px-3 py-2 rounded-xl bg-red-50 border border-red-100 text-red-600 text-xs flex items-center gap-1.5">
                ...
                {error}
                <button onClick={() => setError("")} className="ml-auto hover:text-red-800" aria-label="Fermer">✕</button>
              </div>
            )}
