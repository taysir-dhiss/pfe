...
import { UserIcon, ShieldCheckIcon, UserGroupIcon, BookOpenIcon } from "../../components/Icons";
...
export default function AdminDashboard() {
  ...
  const statCards = [
    { label: "Patientes",  value: stats?.patientCount ?? "—", Icon: UserIcon,       color: "from-pink-50 to-rose-50",    border: "border-pink-200",   text: "text-pink-600",   to: "/admin/patients" },
    { label: "Admins",     value: stats?.adminCount   ?? "—", Icon: ShieldCheckIcon, color: "from-purple-50 to-pink-50", border: "border-purple-200", text: "text-purple-600", to: "#" },
  ];

  const shortcuts = [
    { to: "/admin/patients",     Icon: UserGroupIcon,   label: "Gérer les patientes",  sub: "Créer, modifier, supprimer",  color: "from-pink-50 to-rose-50",    border: "border-pink-200",   text: "text-pink-600"   },
    { to: "/admin/content",      Icon: BookOpenIcon,    label: "BibMed",               sub: "Articles & liens Google",     color: "from-blue-50 to-indigo-50",  border: "border-blue-200",   text: "text-blue-600"   },
    { to: "/admin/rag",          Icon: BookOpenIcon,    label: "Base RAG",             sub: "PDFs indexés pour le chatbot", color: "from-teal-50 to-emerald-50", border: "border-teal-200",   text: "text-teal-600"   },
    { to: "/admin/create-admin", Icon: ShieldCheckIcon, label: "Créer un admin",       sub: "Nouveau compte administrateur", color: "from-purple-50 to-pink-50", border: "border-purple-200", text: "text-purple-600" },
  ];
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 stagger">
        {shortcuts.map(({ to, Icon, label, sub, color, border, text }) => (
          <Link key={to} to={to}
            ...
            <div className="w-12 h-12 rounded-xl bg-white/70 flex items-center justify-center shadow-sm flex-shrink-0 group-hover:scale-110 transition-transform duration-200">
